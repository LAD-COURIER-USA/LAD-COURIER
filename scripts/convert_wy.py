...
db_path = 'scripts/data/geodata-WY.parquet'
csv_output = 'scripts/data/geodata-WY-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de WYOMING...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Wyoming).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-WY', 'WY'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """

        con.execute(sql)
        print(f"✅ ¡Éxito! Wyoming ha sido purificada. Archivo creado en: {csv_output}")
