...
db_path = 'scripts/data/geodata-ID.parquet'
csv_output = 'scripts/data/geodata-ID-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de IDAHO...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Idaho).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-ID', 'ID'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """

        con.execute(sql)
        print(f"✅ ¡Éxito! Idaho ha sido purificada. Archivo creado en: {csv_output}")
