...
db_path = 'scripts/data/geodata-OH.parquet'
csv_output = 'scripts/data/geodata-OH-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de OHIO...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Ohio).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Ohio
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Ohio
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-OH', 'OH')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Ohio ha sido purificada. Archivo creado en: {csv_output}")
