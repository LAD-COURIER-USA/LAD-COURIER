...
db_path = 'scripts/data/geodata-NH.parquet'
csv_output = 'scripts/data/geodata-NH-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de NEW HAMPSHIRE...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de New Hampshire).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-NH', 'NH'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """
        con.execute(sql)
        print(f"✅ ¡Éxito! New Hampshire ha sido purificada. Archivo creado en: {csv_output}")
