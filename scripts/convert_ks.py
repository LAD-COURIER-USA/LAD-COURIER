...
db_path = 'scripts/data/geodata-KS.parquet'
csv_output = 'scripts/data/geodata-KS-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de KANSAS...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Kansas).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Kansas
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Kansas
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-KS', 'KS')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Kansas ha sido purificada. Archivo creado en: {csv_output}")
