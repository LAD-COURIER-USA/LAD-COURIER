...
db_path = 'scripts/data/geodata-MS.parquet'
csv_output = 'scripts/data/geodata-MS-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de MISSISSIPPI...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Mississippi).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Mississippi
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Mississippi
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-MS', 'MS')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Mississippi ha sido purificada. Archivo creado en: {csv_output}")
