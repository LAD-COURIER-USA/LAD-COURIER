...
db_path = 'scripts/data/geodata-WA.parquet'
csv_output = 'scripts/data/geodata-WA-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de WASHINGTON...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Washington).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-WA', 'WA'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """

        con.execute(sql)
        print(f"✅ ¡Éxito! Washington ha sido purificada. Archivo creado en: {csv_output}")
