...
csv_output = 'scripts/data/geodata-GA-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de Georgia...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Georgia).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado: Filtramos por categoría Y por región de forma estricta
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto (Evita que locales de Florida entren en Georgia)
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-GA', 'GA')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Georgia ha sido purificada. Archivo creado en: {csv_output}")
