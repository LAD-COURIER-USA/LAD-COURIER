...
db_path = 'scripts/data/geodata-TX.parquet'
csv_output = 'scripts/data/geodata-TX-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de TEXAS...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Texas).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Texas
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Texas
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-TX', 'TX')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Texas ha sido purificada. Archivo creado en: {csv_output}")
