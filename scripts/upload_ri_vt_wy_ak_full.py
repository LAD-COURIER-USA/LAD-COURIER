...
def split_address_smart(addr):
    if not addr or addr.lower() in ['no address', '']:
        return "", ""
    addr = addr.strip()
    # Prioridad 1: Número al inicio (Ej: 123 Main St)
    match_start = re.match(r'^(\d+)\s+(.*)$', addr)
    if match_start:
        return match_start.group(1), match_start.group(2).upper()
    # Prioridad 2: Cualquier número en la cadena (para formatos raros)
...
def upload_task(name, file_name, collection, state_code):
    ...
    with open(file_path, mode='r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if not num: continue # Sin número no hay ADN LAD
...
def main():
    ...
    # Definición de los 4 estados pequeños
    tasks = [
        {'name': 'Rhode Island', 'file': 'geodata-RI-final.csv', 'coll': 'geodata_us_ri', 'code': 'RI'},
        {'name': 'Vermont', 'file': 'geodata-VT-final.csv', 'coll': 'geodata_us_vt', 'code': 'VT'},
        {'name': 'Wyoming', 'file': 'geodata-WY-final.csv', 'coll': 'geodata_us_wy', 'code': 'WY'},
        {'name': 'Alaska', 'file': 'geodata-AK-final.csv', 'coll': 'geodata_us_ak', 'code': 'AK'},
    ]
