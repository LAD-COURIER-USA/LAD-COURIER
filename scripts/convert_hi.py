...
db_path = 'scripts/data/geodata-HI.parquet'
csv_output = 'scripts/data/geodata-HI-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de HAWAII...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Hawaii).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-HI', 'HI'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """

        con.execute(sql)
        print(f"✅ ¡Éxito! Hawaii ha sido purificada. Archivo creado en: {csv_output}")
