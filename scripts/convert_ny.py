...
db_path = 'scripts/data/geodata-NY.parquet'
csv_output = 'scripts/data/geodata-NY-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de NEW YORK...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de New York).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para New York
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para New York
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-NY', 'NY')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! New York ha sido purificada. Archivo creado en: {csv_output}")
