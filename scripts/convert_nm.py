...
db_path = 'scripts/data/geodata-NM.parquet'
csv_output = 'scripts/data/geodata-NM-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de NEW MEXICO...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de New Mexico).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                -- Filtro Geográfico para New Mexico (NM)
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-NM', 'NM')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! New Mexico ha sido purificada. Archivo creado en: {csv_output}")
