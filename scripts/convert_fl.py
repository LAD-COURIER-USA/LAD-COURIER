...
db_path = 'scripts/data/geodata-FL.parquet'
csv_output = 'scripts/data/geodata-FL-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de FLORIDA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Florida).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-FL', 'FL'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """

        con.execute(sql)
        print(f"✅ ¡Éxito! Florida ha sido purificada. Archivo creado en: {csv_output}")
