...
db_path = 'scripts/data/geodata-KY.parquet'
csv_output = 'scripts/data/geodata-KY-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de KENTUCKY...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Kentucky).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Kentucky
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Kentucky
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-KY', 'KY')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Kentucky ha sido purificada. Archivo creado en: {csv_output}")
