...
db_path = 'scripts/data/geodata-NE.parquet'
csv_output = 'scripts/data/geodata-NE-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de NEBRASKA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Nebraska).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                -- Filtro Geográfico para Nebraska (NE)
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-NE', 'NE')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Nebraska ha sido purificada. Archivo creado en: {csv_output}")
