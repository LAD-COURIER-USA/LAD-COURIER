...
db_path = 'scripts/data/geodata-PA.parquet'
csv_output = 'scripts/data/geodata-PA-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de PENNSYLVANIA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Pennsylvania).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Pennsylvania
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Pennsylvania
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-PA', 'PA')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Pennsylvania ha sido purificada. Archivo creado en: {csv_output}")
