...
db_path = 'scripts/data/geodata-NV.parquet'
csv_output = 'scripts/data/geodata-NV-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de NEVADA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Nevada).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-NV', 'NV'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """

        con.execute(sql)
        print(f"✅ ¡Éxito! Nevada ha sido purificada. Archivo creado en: {csv_output}")
