...
db_path = 'scripts/data/geodata-MO.parquet'
csv_output = 'scripts/data/geodata-MO-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de MISSOURI...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Missouri).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Missouri
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Missouri
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-MO', 'MO')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Missouri ha sido purificada. Archivo creado en: {csv_output}")
