...
db_path = 'scripts/data/geodata-AK.parquet'
csv_output = 'scripts/data/geodata-AK-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de ALASKA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Alaska).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-AK', 'AK'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """

        con.execute(sql)
        print(f"✅ ¡Éxito! Alaska ha sido purificada. Archivo creado en: {csv_output}")
