...
db_path = 'scripts/data/geodata-CA.parquet'
csv_output = 'scripts/data/geodata-CA-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de CALIFORNIA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de California).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-CA', 'CA'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """

        con.execute(sql)
        print(f"✅ ¡Éxito! California ha sido purificada. Archivo creado en: {csv_output}")
