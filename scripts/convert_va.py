...
db_path = 'scripts/data/geodata-VA.parquet'
csv_output = 'scripts/data/geodata-VA-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de VIRGINIA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Virginia).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Virginia
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Virginia
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-VA', 'VA')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Virginia ha sido purificada. Archivo creado en: {csv_output}")
