...
db_path = 'scripts/data/geodata-ME.parquet'
csv_output = 'scripts/data/geodata-ME-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de MAINE...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Maine).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-ME', 'ME'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """
        con.execute(sql)
        print(f"✅ ¡Éxito! Maine ha sido purificada. Archivo creado en: {csv_output}")
