...
db_path = 'scripts/data/geodata-ND.parquet'
csv_output = 'scripts/data/geodata-ND-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de NORTH DAKOTA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de North Dakota).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                -- Filtro Geográfico para North Dakota (ND)
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-ND', 'ND')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! North Dakota ha sido purificada. Archivo creado en: {csv_output}")
