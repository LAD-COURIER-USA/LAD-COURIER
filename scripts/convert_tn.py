...
db_path = 'scripts/data/geodata-TN.parquet'
csv_output = 'scripts/data/geodata-TN-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de TENNESSEE...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Tennessee).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Tennessee
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Tennessee
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-TN', 'TN')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Tennessee ha sido purificada. Archivo creado en: {csv_output}")
