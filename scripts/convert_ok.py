...
db_path = 'scripts/data/geodata-OK.parquet'
csv_output = 'scripts/data/geodata-OK-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de OKLAHOMA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Oklahoma).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Oklahoma
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Oklahoma
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-OK', 'OK')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Oklahoma ha sido purificada. Archivo creado en: {csv_output}")
