...
db_path = 'scripts/data/geodata-NJ.parquet'
csv_output = 'scripts/data/geodata-NJ-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de NEW JERSEY...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de New Jersey).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-NJ', 'NJ'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """
        con.execute(sql)
        print(f"✅ ¡Éxito! New Jersey ha sido purificada. Archivo creado en: {csv_output}")
