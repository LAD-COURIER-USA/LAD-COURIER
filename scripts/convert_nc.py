...
db_path = 'scripts/data/geodata-NC.parquet'
csv_output = 'scripts/data/geodata-NC-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de NORTH CAROLINA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de North Carolina).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para North Carolina
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para North Carolina
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-NC', 'NC')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! North Carolina ha sido purificada. Archivo creado en: {csv_output}")
