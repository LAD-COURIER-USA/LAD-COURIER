...
db_path = 'scripts/data/geodata-MT.parquet'
csv_output = 'scripts/data/geodata-MT-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de MONTANA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Montana).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-MT', 'MT'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """

        con.execute(sql)
        print(f"✅ ¡Éxito! Montana ha sido purificada. Archivo creado en: {csv_output}")
