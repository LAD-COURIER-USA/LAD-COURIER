...
db_path = 'scripts/data/geodata-SD.parquet'
csv_output = 'scripts/data/geodata-SD-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de SOUTH DAKOTA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de South Dakota).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                -- Filtro Geográfico para South Dakota (SD)
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-SD', 'SD')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! South Dakota ha sido purificada. Archivo creado en: {csv_output}")
