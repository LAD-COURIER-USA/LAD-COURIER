...
db_path = 'scripts/data/geodata-MA.parquet'
csv_output = 'scripts/data/geodata-MA-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de MASSACHUSETTS...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Massachusetts).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-MA', 'MA'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """
        con.execute(sql)
        print(f"✅ ¡Éxito! Massachusetts ha sido purificada. Archivo creado en: {csv_output}")
