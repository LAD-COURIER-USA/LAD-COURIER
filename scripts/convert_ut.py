...
db_path = 'scripts/data/geodata-UT.parquet'
csv_output = 'scripts/data/geodata-UT-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de UTAH...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Utah).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-UT', 'UT'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """

        con.execute(sql)
        print(f"✅ ¡Éxito! Utah ha sido purificada. Archivo creado en: {csv_output}")
