...
def split_address_smart(addr):
    ...
    # Prioridad 2: Cualquier número en la cadena
...
def upload_task(name, file_name, collection, state_code):
    ...
    print(f"\n📦 PROCESANDO EL GRAN FINAL: {name} ({state_code}) -> {collection}")
    ...
    with open(file_path, mode='r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if count % 500 == 0:
                batch.commit()
                batch = db.batch()
                print(f"  ⚡ {count} locales inyectados en Florida...")
...
def main():
    ...
    # Solo Florida
    upload_task('Florida', 'geodata-FL-final.csv', 'geodata_us_fl', 'FL')
