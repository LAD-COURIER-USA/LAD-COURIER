...
db_path = 'scripts/data/geodata-LA.parquet'
csv_output = 'scripts/data/geodata-LA-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de LOUISIANA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Louisiana).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Louisiana
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Louisiana
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-LA', 'LA')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Louisiana ha sido purificada. Archivo creado en: {csv_output}")
