...
db_path = 'scripts/data/geodata-RI.parquet'
csv_output = 'scripts/data/geodata-RI-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de RHODE ISLAND...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Rhode Island).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-RI', 'RI'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """
        con.execute(sql)
        print(f"✅ ¡Éxito! Rhode Island ha sido purificada. Archivo creado en: {csv_output}")
