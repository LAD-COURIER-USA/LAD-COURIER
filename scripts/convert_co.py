...
db_path = 'scripts/data/geodata-CO.parquet'
csv_output = 'scripts/data/geodata-CO-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de COLORADO...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Colorado).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                -- Filtro Geográfico para Colorado (CO)
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-CO', 'CO')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Colorado ha sido purificada. Archivo creado en: {csv_output}")
