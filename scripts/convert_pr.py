...
db_path = 'scripts/data/geodata-PR.parquet'
csv_output = 'scripts/data/geodata-PR-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de PUERTO RICO...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Puerto Rico).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-PR', 'PR'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """

        con.execute(sql)
        print(f"✅ ¡Éxito! Puerto Rico ha sido purificada. Archivo creado en: {csv_output}")
