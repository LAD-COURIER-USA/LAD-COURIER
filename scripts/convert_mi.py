...
db_path = 'scripts/data/geodata-MI.parquet'
csv_output = 'scripts/data/geodata-MI-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de MICHIGAN...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Michigan).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Michigan
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico para Michigan (MI)
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-MI', 'MI')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Michigan ha sido purificada. Archivo creado en: {csv_output}")
