...
db_path = 'scripts/data/geodata-WI.parquet'
csv_output = 'scripts/data/geodata-WI-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de WISCONSIN...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Wisconsin).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Wisconsin
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico para Wisconsin (WI)
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-WI', 'WI')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Wisconsin ha sido purificada. Archivo creado en: {csv_output}")
