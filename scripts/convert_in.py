...
db_path = 'scripts/data/geodata-IN.parquet'
csv_output = 'scripts/data/geodata-IN-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de INDIANA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Indiana).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Indiana
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Indiana
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-IN', 'IN')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Indiana ha sido purificada. Archivo creado en: {csv_output}")
