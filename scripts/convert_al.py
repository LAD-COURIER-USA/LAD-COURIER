...
db_path = 'scripts/data/geodata-AL.parquet'
csv_output = 'scripts/data/geodata-AL-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de ALABAMA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Alabama).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Alabama
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Alabama
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-AL', 'AL')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Alabama ha sido purificada. Archivo creado en: {csv_output}")
