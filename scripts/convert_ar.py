...
db_path = 'scripts/data/geodata-AR.parquet'
csv_output = 'scripts/data/geodata-AR-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de ARKANSAS...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Arkansas).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Arkansas
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Arkansas
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-AR', 'AR')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Arkansas ha sido purificada. Archivo creado en: {csv_output}")
