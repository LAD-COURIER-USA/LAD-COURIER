...
db_path = 'scripts/data/geodata-IL.parquet'
csv_output = 'scripts/data/geodata-IL-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de ILLINOIS...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Illinois).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Illinois
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para Illinois
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-IL', 'IL')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Illinois ha sido purificada. Archivo creado en: {csv_output}")
