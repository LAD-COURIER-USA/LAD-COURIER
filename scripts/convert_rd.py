...
db_path = 'scripts/data/geodata-RD.parquet'
csv_output = 'scripts/data/geodata-RD-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de REPUBLICA DOMINICANA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de RD).")
    ...
    try:
        ...
        COPY (
            SELECT 
                ...
                1 AS location_count,
                'República Dominicana' AS market_name,
            ...
            WHERE 
                -- Filtro por país DO
                (json_extract_string(CAST(addresses AS JSON), '$[0].country') = 'DO' OR country = 'DO')
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! República Dominicana ha sido purificada. Archivo creado en: {csv_output}")
