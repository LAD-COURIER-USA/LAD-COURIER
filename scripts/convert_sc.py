...
db_path = 'scripts/data/geodata-SC.parquet'
csv_output = 'scripts/data/geodata-SC-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de SOUTH CAROLINA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de South Carolina).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para South Carolina
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para South Carolina
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-SC', 'SC')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! South Carolina ha sido purificada. Archivo creado en: {csv_output}")
