...
db_path = 'scripts/data/geodata-VT.parquet'
csv_output = 'scripts/data/geodata-VT-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de VERMONT...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Vermont).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-VT', 'VT'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """
        con.execute(sql)
        print(f"✅ ¡Éxito! Vermont ha sido purificada. Archivo creado en: {csv_output}")
