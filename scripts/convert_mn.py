...
db_path = 'scripts/data/geodata-MN.parquet'
csv_output = 'scripts/data/geodata-MN-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de MINNESOTA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Minnesota).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Minnesota
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico para Minnesota (MN)
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-MN', 'MN')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Minnesota ha sido purificada. Archivo creado en: {csv_output}")
