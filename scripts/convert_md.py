...
db_path = 'scripts/data/geodata-MD.parquet'
csv_output = 'scripts/data/geodata-MD-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de MARYLAND...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Maryland y Delaware).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Maryland y Delaware (incluidos en el mismo bbox)
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico para Maryland (MD) y Delaware (DE)
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-MD', 'MD', 'US-DE', 'DE')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Maryland/Delaware ha sido purificada. Archivo creado en: {csv_output}")
