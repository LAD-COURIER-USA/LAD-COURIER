...
db_path = 'scripts/data/geodata-IA.parquet'
csv_output = 'scripts/data/geodata-IA-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de IOWA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Iowa).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para Iowa
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico para Iowa (IA)
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-IA', 'IA')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! Iowa ha sido purificada. Archivo creado en: {csv_output}")
