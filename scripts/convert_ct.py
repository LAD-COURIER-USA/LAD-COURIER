...
db_path = 'scripts/data/geodata-CT.parquet'
csv_output = 'scripts/data/geodata-CT-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de CONNECTICUT...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de Connecticut).")
    ...
    try:
        ...
        COPY (
            ...
            WHERE 
                (json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-CT', 'CT'))
                AND names IS NOT NULL
        ) TO '{csv_output}' (FORMAT CSV, HEADER);
        """
        con.execute(sql)
        print(f"✅ ¡Éxito! Connecticut ha sido purificada. Archivo creado en: {csv_output}")
