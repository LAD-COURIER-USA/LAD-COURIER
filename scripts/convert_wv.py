...
db_path = 'scripts/data/geodata-WV.parquet'
csv_output = 'scripts/data/geodata-WV-final.csv'
...
def main():
    ...
    print("🚀 Iniciando purificación y conversión de WEST VIRGINIA...")
    print("🌪️ MODO ASPIRADORA ACTIVADO (Sin filtros de categoría)\nFiltro geográfico: Estricto (Solo locales dentro de West Virginia).")

    try:
        con = duckdb.connect()
        con.execute("INSTALL spatial; LOAD spatial;")

        # SQL Purificado para West Virginia
        sql = f"""
        COPY (
            SELECT 
                id AS place_id,
            ...
            WHERE 
                -- Filtro Geográfico Estricto para West Virginia
                (
                    json_extract_string(CAST(addresses AS JSON), '$[0].region') IN ('US-WV', 'WV')
                )
                AND 
        ...
        con.execute(sql)
        print(f"✅ ¡Éxito! West Virginia ha sido purificada. Archivo creado en: {csv_output}")
